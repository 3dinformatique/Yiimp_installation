...
#   After a bug, added php7.3-memcache package
################################################################################
    ...
    sudo apt install -y dialog python3 python3-pip acl nano apt-transport-https
    ...
    PRIVATE_IP1=$(ip addr show | grep "inet " | grep "/24" | awk '{print $2}')
